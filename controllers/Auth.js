...
exports.register = asyncHandler(async (req, res, next) => {
    ...
    if (newUser.father) {
        ...
        if (!father.valid) {
            return next(new ErrorResponse(`father id '${newUser.father}' not registered to any account`, 400))
        }
        ...
        if (father.user.type !== "parent") {
            return next(new ErrorResponse(`father provided with id of ${newUser.father} is not registered as a parent`, 400))
        }
    }
    ...
    if (newUser.mother) {
        ...
        if (!mother.valid) {
            return next(new ErrorResponse(`mother id '${newUser.mother}' not registered to any account`, 400))
        }
        ...
        if (mother.user.type !== "parent") {
            return next(new ErrorResponse(`mother provided with id of ${newUser.mother} is not registered as a parent`, 400))
        }
    }
...
exports.updatePassword = asyncHandler(async (req, res, next) => {
    ...
    if (!currentPassword || !newPassword) {
        return next(new ErrorResponse(`add current password and new password`, 400))
    }
